...
fun <T> CommandContext<T>.world(id: String = "world", origin: Location? = null): String {
    return if (get(id) == "~") (origin ?: player().location).world!!.name else get(id)
}
...
fun <T> CommandContext<T>.worldOrNull(id: String = "world", origin: Location? = null): String? {
    ...
    return if (world == "~") (origin ?: player().location).world?.name else world
}
...
fun <T> CommandContext<T>.location(
    ...
    return Location(Bukkit.getWorld(world(world, origin)) ?: error("World not found: ${world(world, origin)}"), x(x, origin), y(y, origin), z(z, origin), yawOrNull(yaw, origin) ?: 0f, pitchOrNull(pitch, origin) ?: 0f)
}
