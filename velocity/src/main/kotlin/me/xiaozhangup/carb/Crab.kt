...
class Crab(anchor: Class<*>, dataFolder: () -> File, private val plugin: () -> Any) :
    ...
    fun <E : Any> fireEvent(event: E) = me.xiaozhangup.carb.CarbKotlin.getServer().eventManager.fire(event)
    fun onlinePlayers() = me.xiaozhangup.carb.CarbKotlin.getServer().allPlayers
    fun console() = me.xiaozhangup.carb.CarbKotlin.getServer().consoleCommandSource
    fun executeConsole(command: String) =
        me.xiaozhangup.carb.CarbKotlin.getServer().commandManager.executeAsync(console(), command)
