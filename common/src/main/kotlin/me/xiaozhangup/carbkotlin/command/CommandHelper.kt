...
import me.xiaozhangup.carbkotlin.command.component.CommandComponent
import me.xiaozhangup.carbkotlin.command.component.CommandComponentDynamic
import me.xiaozhangup.carbkotlin.command.component.CommandComponentLiteral

fun CommandComponent.createHelper(checkPermissions: Boolean = true) {
    execute<Audience> { sender, context, _ ->
        ...
        fun check(children: List<CommandComponent>): List<CommandComponent> {
            ...
            val filterChildren = if (checkPermissions) {
                children.filter { (it.permission.isEmpty() || context.checkPermission(it.permission)) }
            } else {
                children
            }
            // 过滤隐藏
            ...
            when (compound) {
                ...
                is CommandComponentDynamic -> {
                    val value = if (compound.comment.startsWith("@")) {
                        context.command.registry.languageResolver(sender, compound.comment.substring(1))
                    } else {
                        compound.comment
                    }
        ...
        builder.lines().forEach {
            CommandMessages.usage(sender, command, it)
        }
    }
}
...
fun CommandComponent.createDescriptionHelper(checkPermissions: Boolean = true) {
    execute<Audience> { sender, context, _ ->
        ...
        val rootChildren = context.commandCompound.children
            .filter { !checkPermissions || (it.permission.isEmpty() || context.checkPermission(it.permission)) }
            .filter { it !is CommandComponentLiteral || !it.hidden }
        val text =
            CommandHelper.buildDescriptionHelperText(command.name, rootChildren) { key -> context.command.registry.languageResolver(sender, key) }
        text.lines().forEach { CommandMessages.usage(sender, command, it) }
    }
}
