...
abstract class CommandComponent(val index: Int, var optional: Boolean, val permission: String = "") {
    ...
    /** Detect a denied branch without invoking its suggestions, restrictions or executor. */
    internal fun findDeniedChild(context: CommandContext<*>, parameter: String): CommandComponent? {
        return children.firstOrNull {
            it.permission.isNotEmpty() && !context.checkPermission(it.permission) && when (it) {
                is CommandComponentLiteral -> it.aliases.any { alias -> alias.equals(parameter, true) }
                is CommandComponentDynamic -> parameter.isNotEmpty()
                else -> false
            }
        }
    }
