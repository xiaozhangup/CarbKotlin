...
class SingleRedisConnector: Closeable {
    ...
    private val sharedConnection by lazy { SingleRedisConnection(pool ?: error("connect first"), this) }
     ...
     */
    override fun close() {
        if (pool != null) sharedConnection.close()
    }

    internal fun closePool() { pool?.destroy() }
