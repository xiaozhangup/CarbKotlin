...
import me.xiaozhangup.carb.reflect.Reflex.Companion.setProperty
...
/**
 * TabooLib me.xiaozhangup.carb.configuration.ConfigFile
 *
...
open class ConfigFile(root: Config) : ConfigSection(root), Configuration {
    ...
    override fun loadFromFile(file: File) {
        ...
        try {
            ...
            LoggerFactory.getLogger(ConfigFile::class.java).warn("File: $file")
            throw ex
        }
    ...
    override fun loadFromString(contents: String) {
        try {
            ...
            LoggerFactory.getLogger(ConfigFile::class.java).warn("Source: \n$contents")
            throw t
        }
        reloadGenerationCounter.incrementAndGet()
        reloadCallback.forEach { it.run() }
